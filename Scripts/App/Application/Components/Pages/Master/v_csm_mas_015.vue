                        ...
                        <table class="table table-hover table-bordered">
                          <thead>
                            <tr>
                              ...
                              <th class="tf-5-5">Description</th>
                              <th class="tf-3">Group</th>
                              <th class="tf-4">Reference</th>
                          ...
                          <tbody>
                            <tr v-for="x,idx in displayData">
                              ...
                              <td align="center"><b>{{x.descode}}</b></td>
                              <td>{{x.desname}}</td>
                              <td align="center" class="text-bold">{{x.desgroup}}</td>
                              <td align="center">{{x.refcode}}</td>
                              <td>{{x.desname2}}</td>
                      ...
                      <div class="row">
                        <div class="col-md-1">
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control" v-model="form.descode" readonly />
                          </div>
                        </div>
                        <div class="col-md-1">
                          <div class="form-group">
                      ...
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control" v-model="form.desname" maxlength="100" />
                          </div>
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group">
                            <label>Remark</label>
                            <input type="text" class="form-control" v-model="form.desname2" maxlength="200" />
                          </div>
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-md-2">
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control" v-model="form.refcode" maxlength="20" />
                          </div>
                        </div>
                      </div>
                    </template>
...
<script>
  ...
  let cpn = {
    ...
    methods: {
      ...
      async loadData() {
        let act = `anywhere/master/Description_ReadList?type=CSM&text=${encodeURIComponent(this.search.text || '')}`;
      ...
      async readData(x) {
        let act = `anywhere/master/Description_Read?descode=${encodeURIComponent(x || '')}`;
        let rsp = await $xt.getServer(act);
        this.form = rsp;
        this.setEdit();
      },
      async save() {
        ...
        try {
          ...
          let act = `anywhere/master/Description_Create`;
          if (this.editMode) {
            act = `anywhere/master/Description_Update`;
          }
      ...
      async deleteData() {
        ...
        page.loadingBox.show();
        let act = `Anywhere/master/Description_Delete?descode=${this.form.descode || ''}`;
        let rsp = await $xt.getServer(act);
        if (!rsp.success) {
