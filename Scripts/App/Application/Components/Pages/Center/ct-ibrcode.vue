<template>
  <div>
    <div class="modal fade" ref="MaterialModal">
      <div class="modal-dialog" style="width:70% !important;">
        <div class="modal-content">
          <div class="modal-header">
            <h4>ข้อมูลรายการสินค้าหรือวัสดุอุปกรณ์</h4>
          </div>
                  ...
                  <table class="table table-bordered table-striped table-hover">
                    <thead>
                      <tr class="text-center">
                        <th class="tf-3-5">Material Code</th>
                        <th class="tf-3-5">Material Group</th>
                        <th class="tf-5">Material Name</th>
                        <th class="tf-4">Spec/Size</th>
                        <th class="tf-4">Brand</th>
                        <th class="tf-2-5">Unit</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(x,idx) in respData" style="cursor:pointer" @click="sendData(x)">
                        <td class="text-bold">{{x.type_code}}</td>
                        <td>{{x.c_des}}</td>
                        <td>{{x.c_des1}}</td>
                        <td>{{x.c_des2}}</td>
                        <td>{{x.c_des3}}</td>
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      openModal() {
        (async () => {
          $(this.$refs.MaterialModal).modal('show');
