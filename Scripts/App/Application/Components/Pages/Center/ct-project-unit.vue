<template>
  <div>
    <div class="modal fade" ref="ProjectUnitModal">
      <div class="modal-dialog" style="width:70% !important;">
        <div class="modal-content">
          <div class="modal-header">
            <h4>Project (Unit/Phase)</h4>
          </div>
                  ...
                  <table class="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                      <tr v-for="(x,idx) in respData" style="cursor:pointer" @click="sendData(x)">
                        <td align="center" class="text-bold">{{x.pre_event}}</td>
                        <td align="center" class="text-bold">{{x.refcode}}</td>
                        <td>
                          {{x.pre_des}}
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      openModal() {
        (async () => {
          $(this.$refs.ProjectUnitModal).modal('show');
          await this.loadData();
        })();
      },
      closeModal() {
        $(this.$refs.ProjectUnitModal).modal('hide');
      },
      pageChange(pn) {
        (async () => {
      ...
      loadData() {
        (async () => {
          let url = `CSM/Center/UnitPhase_ReadList?skip=${paging.skipItems()}&take=${paging.getItemsPerPage()}&pre_event2=${this.pre_event2 || ''}`;
